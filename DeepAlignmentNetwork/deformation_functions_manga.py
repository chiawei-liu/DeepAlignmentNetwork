...
def deform_nose(lms, p_scale=0, p_shift=0, pad=5):
    ...
    nose_inds = np.arange(59,60)
    left_eye_inds = np.append(np.arange(27, 37), 57)
    right_eye_inds = np.append(np.arange(37, 47), 58)
    mouth_inds = np.arange(17, 27)
    ...
    if p_shift > 0.5 and (np.random.rand() > 0.5 or not scale):
        ...
        error2 = check_deformation_spatial_errors(lms_def, part_inds[:], pad=pad)
        error = error1 + error2
        if error:
            lms_def = lms_def_scale.copy()
    else:
...
def deform_eyes(lms, p_scale=0, p_shift=0, pad=10):
    ...
    y_max_right = np.max(lms[part_inds_right, 0]) + (lms[59, 0] - np.max(lms[part_inds_right, 0])) * 0.25 - pad
    y_min_right = 2 * pad
    ...
    y_max_left = np.max(lms[part_inds_left, 0]) + (lms[59, 0] - np.max(lms[part_inds_left, 0])) * 0.25 - pad
    y_min_left = 2 * pad
...
def deform_scale_face(lms, p_scale=0, pad=5, image_size=256):
    ...
    if p_scale > 0.5:
        ...
        error1 = not each_unique(lms_def_scale.astype('int'))
        error = error1 + error2
        if error:
            lms_def_scale = lms.copy()
    else:
...
def each_unique(landmarks):
    for i in range(landmarks.shape[0]):
        for j in range(i):
            if np.array_equal(landmarks[i], landmarks[j]):
                return False
    return True